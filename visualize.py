...
# KONFIGURÁCIA
# ─────────────────────────────────────────────────────────────────────────────
json_path = "data/cache/pipeline/dbbf8dde-1d40-481b-ba5f-4d84c2de3e54_final_old.json"
...
<head>
<meta charset="UTF-8">
<title>Strana {page_number} — Doc Viewer</title>
...
<div class="diff-legend">
  <span class="diff-item"><span class="diff-box-agreed"></span>GAT = YOLO (zhodli sa)</span>
  <span class="diff-item"><span class="diff-box-changed"></span>GAT ≠ YOLO (GAT opravil)</span>
  <span class="diff-item"><span class="diff-box-table">⊞</span>Kliknúť = zobraziť tabuľku</span>
</div>
  ...
  <div class="table-side-panel" id="tableSidePanel">
    <div class="tsp-header">
      <div class="tsp-title">
        ...
        <span id="tspTitle">Tabuľka</span>
      </div>
      <div class="tsp-actions">
        <button class="tsp-btn tsp-btn-copy" id="tspCopyBtn" onclick="copyTableForNotion()">
          📋 Kopírovať pre Notion
        </button>
...
function showSidePanel(boxEl) {{
  // Prečítame bezpečný Base64 reťazec
  const b64Html = boxEl.getAttribute("data-table-b64") || "";
  const titleText = boxEl.getAttribute("data-table-title") || "Tabuľka";

  let tableHtml = "";
  if (b64Html) {{
    try {{
      // Dekódujeme Base64 bezpečne späť na UTF-8 HTML
      tableHtml = decodeURIComponent(escape(window.atob(b64Html)));
    }} catch (e) {{
      console.error("Chyba pri dekódovaní Base64 tabuľky:", e);
    }}
  }}
  ...
  // If same box clicked again → toggle close
  ...
  if (tableHtml) {{
    ...
    tspBody.innerHTML = '<div class="tsp-empty">Tabuľka neobsahuje extrahované dáta.</div>';
  }}
...
function resetCopyBtn() {{
  tspCopyBtn.textContent = "📋 Kopírovať pre Notion";
...
async function copyTableForNotion() {{
  ...
  // Wrap in a clean HTML document fragment for clipboard
  const fullHtml = `<html><body>${{currentTableHtml}}</body></html>`;

  try {{
    // Modern clipboard API — Notion reads text/html directly
    const blob = new Blob([fullHtml], {{ type: "text/html" }});
    const item = new ClipboardItem({{ "text/html": blob }});
    await navigator.clipboard.write([item]);
  }} catch (e) {{
    // Fallback: execCommand with a temporary contenteditable div
  ...
  tspCopyBtn.textContent = "✓ Skopírované!";
...
for page_data in document_data["pages"]:
    ...
    print(f"Generujem stranu {page_number}...")
    ...
    stats_html = (
        ...
        f'<div class="stat hi">✓ Zhodli sa: <b>{len(agreed_nodes)}</b></div>'
        f'<div class="stat" style="color:var(--changed)">✦ GAT opravil: <b>{len(changed_nodes)}</b></div>'
        f'<div class="stat" style="color:#38bdf8">⊞ Tabuľky: <b>{len(table_nodes)}</b></div>'
    ...
    for node in nodes_list:
        ...
        if is_table:
            ...
            text_preview = f'<div class="tt-text">{extracted_text or "<i>(bez textu)</i>"}</div>'

            # Generuj chip pre tabuľku
            gat_label, yolo_label = get_node_label(node)
            changed = (gat_label != yolo_label)
            box_color = CATEGORY_COLORS.get(gat_label, CATEGORY_COLORS["Unknown"])
            yolo_color = CATEGORY_COLORS.get(yolo_label, CATEGORY_COLORS["Unknown"])
            ...
            # Generuj tooltip
            status_tag = (
                f'<span class="tt-tag tt-changed">GAT OPRAVIL</span>' if changed
                else f'<span class="tt-tag tt-agreed">ZHODLI SA</span>'
            )
            ...
            diff_block = (
                ...
                f'<div class="diff-block-title">{"Klasifikácia — ROZDIEL" if changed else "Klasifikácia"}</div>'
            )
            diff_block += (
            ...
            diff_block += (
                ...
                f'{bar(yolo_conf_val, yolo_color)}'
            ...
            tooltip = (
                ...
                f'<span style="font-family:var(--mono);font-size:9px;color:var(--muted)">uzol #{node_id}</span>'
                f'{status_tag}'
                f'</div>'
                f'{diff_block}'
            ...
            # Generuj box HTML s chipom a tooltipom
            css_classes = f'bbox {"changed" if changed else "agreed"}'
            # Extrakcia priamo z tvojho JSON formátu
            # Extrakcia priamo z tvojho JSON formátu
            # Extrakcia priamo z tvojho JSON formátu
            ...
            # Ak tabuľka chýba, dáme aspoň čistý text
            ...
            # 100% BEZPEČNÝ PRENOS DO JS CEZ BASE64
            if table_html:
                # Zakódujeme do UTF-8 a následne do base64 stringu
            ...
            boxes_html += (
                ...
                f' data-table-title="Tabuľka #{table_counter}"'
